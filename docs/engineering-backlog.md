...
| `[ ]` | **State machine** for vehicle state (idle / armed / guided / auto / RTL / fault) | 🔴 | Consumes heartbeats, exposes a stable state to consumers (perception, voice, LEDs). |
| `[ ]` | **Path planner** — reactive obstacle-aware command generation | 🔴 | Start with sector-based potential-fields avoidance (simple); A* / RRT* are §6.6 work. Consumes `depth_detect.py` events, emits velocity / position-target offboard commands. |
| `[ ]` | **End-to-end closed loop**: depth_detect → bridge → SITL (with obstacle) → avoid → resume | 🔴 | The T5 collision-avoidance test (§ 5.4). Validates everything above. |
