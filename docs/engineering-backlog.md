...
| `[ ]` | **Bridge: request extended MAVLink streams on connect** | 🟢 | After `wait_heartbeat` in `bridge.Vehicle.connect()`, send `REQUEST_DATA_STREAM` for `MAV_DATA_STREAM_EXTRA1/2/3` and `MAV_DATA_STREAM_EXTENDED_STATUS` at 4–10 Hz. ArduPilot defaults to low stream rates over USB-MAVLink — the airframe walkaround on 2026-05-18 showed `alt=— gs=— sats=— batt=—` in every STATUS line despite FSM correctly reaching `NO_FIX` (GPS_RAW_INT flowing, `SYS_STATUS` / `VFR_HUD` / extended GPS not). Affects observability only — not flight safety. |
