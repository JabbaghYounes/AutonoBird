...
| `[x]` | **End-to-end closed loop**: bridge → planner → SITL → avoid → resume | 🔴 | ✅ 2026-05-16. `scripts/autonomy/test_avoidance.py`. PASS: drone in SITL took off to 5 m, cruised N at 1.5 m/s, sidestepped E ~10 m on synthetic obstacle injection at t+8 s, resumed N cruise at t+16 s when obstacle cleared, RTL + land. The T5 collision-avoidance test in the SITL track of § 5.4 is now demonstrated. Real-world / hardware T5 still pending. |
