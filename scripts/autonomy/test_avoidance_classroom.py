"""
scripts/autonomy/test_avoidance_classroom.py

Indoor closed-loop avoidance test against ArduPilot SITL + Gazebo Harmonic,
flown inside the cs_classroom world (a Gazebo replica of the AutonoBird
demo room).

Same closed-loop architecture as test_avoidance_gazebo.py — Gazebo lidar
-> perception bridge -> DepthDetectSource -> Planner -> Vehicle -> SITL
-> Gazebo. The differences are the flight envelope (much smaller indoors)
and the obstacle position (2 m north of drone takeoff vs 12 m outdoors).

The drone spawns at world (-1.5, -3.0) facing north; takeoff to 1.5 m AGL;
cruise north at 0.7 m/s; encounter the 0.25 m radius cylinder at 2 m N
(world y = -1.0); sidestep east; continue to 4 m N (world y = +1.0);
RTL + land + disarm.

Pre-flight:
    1. Gazebo:  gz sim -v4 -r cs_classroom.sdf
...
Run:
    python test_avoidance_classroom.py
"""
...
from bridge import BridgeError, Vehicle, load_config as load_bridge_config  # type: ignore[reportMissingImports]  # noqa: E402
...
# Indoor flight envelope — smaller numbers throughout.
CRUISE_ALT_M = 1.5
PLANNER_RATE_HZ = 10.0
CRUISE_SPEED_MS = 0.7
AVOIDANCE_SPEED_MS = 0.7
OBSTACLE_THRESHOLD_M = 1.2
CLEAR_THRESHOLD_M = 2.0
TOTAL_DURATION_S = 50.0
SETTLE_S = 3.0

# Obstacle in drone-relative NED frame: drone takeoff is at world
# (-1.5, -3.0); obstacle is at world (-1.5, -1.0); so it sits 2 m N of
# the drone with zero E offset. Cylinder radius matches cs_classroom.sdf.
OBSTACLE_N_M = 2.0
OBSTACLE_E_M = 0.0
...
def write_outputs(summary: TestSummary, out_dir: Path, stamp: str) -> tuple[Path, Path | None]:
    ...
    json_path = out_dir / f"classroom_{stamp}.json"
    payload = {
        "test": "classroom_closed_loop_avoidance",
        "timestamp": stamp,
    ...
    ax.set_title("Trajectory vs obstacle (drone frame)")
    ...
    ax.axhline(summary.obstacle_n_m, color="r", linestyle="--", linewidth=1, alpha=0.6, label=f"obstacle @ {summary.obstacle_n_m:.1f} m N")
    ax.axhline(summary.target_north_m, color="g", linestyle="--", linewidth=1, alpha=0.6, label=f"target {summary.target_north_m:.1f} m N")
    ax.set_xlabel("t since planner start (s)")
    ...
    fig.suptitle(f"Indoor cs_classroom closed-loop avoidance — {stamp}")
    fig.tight_layout()
    png_path = out_dir / f"classroom_{stamp}.png"
...
def run() -> int:
    ...
    try:
        ...
        log("GUIDED + arm + takeoff (indoor envelope, 1.5 m AGL) ...")
        v.set_mode("GUIDED")
        arm_with_retry(v)
        ...
        print()
        log("=" * 64)
        log("Indoor cs_classroom closed-loop avoidance — results")
        log("=" * 64)
        ...
        log(f"  PASS passed obstacle (N >= {TARGET_NORTH_M:.1f}) :  "
        ...
        if all_pass:
            log("PASS: indoor cs_classroom closed-loop avoidance demonstrated.")
            return 0
        log("FAIL: indoor formal criteria not met (clearance or planner states).")
        return 1
